...
nx = [
    2058.092,1970.09,1529.60,1128.64,1064.14,
    1013.98,852.11,768.19,706.52,656.27,
    643.85,632.80,589.29,587.56,546.07,
    486.13,479.99,435.84,404.66,388.865,
    365.01
]
...
c = conn.cursor()
c.execute('select * from t_glass_spec_hikari')
fetched = c.fetchall()

sql = "create table t_glass_spec_hikari3 ({0})".format(keys)
c.execute('drop table t_glass_spec_hikari3')
c.execute(sql)
...
for i in range(len(fetched)):
    ...
    s = "insert into t_glass_spec_hikari3 VALUES({0})".format(values)
    c.execute(s)
